---
layout:     post
title:      Improving Felix's Postgresql state machine
...
I brough few improvements to Felix's implementation but before reading the following I recommend you to read carefully [the original post](https://felixge.de/2017/07/27/implementing-state-machines-in-postgresql.html).

**TL;DR** Here is the changes I did:
...
To keep it simple I'll use exactly the same FSM graph then Felix: an ordering process with payment and shipment steps.

![](/assets/img/pgfsm-graph-v1.png)

### Types optimization

First I replaced the storage of states/events from `text` to `enum`. Because I have a finite number of states ans transitions I can properly store them as custom `enum`. It reduces the storage of these to a lighter and constant 4 bytes.
